...
nc = Netcat(conf.flume_uu_asterisk['host'],conf.flume_uu_asterisk['port'])
...
while True:
    ...
    if line == "":
        ...
        session.execute("""INSERT INTO phone_log (id,source,datetime_call,year,month,day,call_a,call_b,call_c,duration,call_inner,in_out) 
            VALUES(UUID(),%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) USING TTL 31536000;""", 
            (conf.pref_uu,dt,year,month,day,d["call_a"],"",d["call_c"],d["duration"],d["inner"],d["in_out"]))
